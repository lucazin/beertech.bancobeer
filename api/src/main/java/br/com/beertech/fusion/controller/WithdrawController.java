...
public class WithdrawController {
	...
	@Autowired
	private PublishTransaction publishTransaction;
	...
	public ResponseEntity<Void> queueWithdrawal(@RequestBody OperationDTO operationDTO,
			@RequestHeader(value = "Authorization", required = false) String token) throws FusionException {
		
		userService.validateUserLogged(token, operationDTO.getHash());

		publishTransaction.publishOperation(new OperationDTO(OperationType.SAQUE, operationDTO.getValorOperacao(),
				operationDTO.getHash(), DebitCreditType.DEBITO, token));
